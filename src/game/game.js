...
export const gameLoop = (world, ctx, delta) => {
    ...
    if (world.entities.length === 0 && !world.gameOver) {
        ...
        spawnLevelSystem(world, levels[currentLevel]);
    ...
    if (!world.gameOver) {
        // --- Update ---
        enemyPatrolSystem(world.entities, world.components, delta);
        ...
        world.entities = world.entities.filter(id => !world.components.collectible?.[id]?.collected);
        ...
        if (playerPos && playerPos.y > 700) world.gameOver = true;

        // --- Check level complete ---
        const levelComplete = !world.entities.some(id =>
    ...
    // --- Render ---
renderSystem(ctx, world.entities, world.components, camera);

// --- Next Level Overlay ---
if (nextLevelTimer > 0) {
    // Poluprozirni crni overlay preko canvas-a
    ctx.fillStyle = 'rgba(0,0,0,0.5)';
    ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);

    // Tekst u sredini ekrana
    ctx.fillStyle = 'white';
    ctx.font = '48px Arial';
    ctx.textAlign = 'center';
    ctx.fillText(`NEXT LEVEL ${currentLevel + 1}`, ctx.canvas.width / 2, ctx.canvas.height / 2);

    nextLevelTimer--; // odbrojavanje
} 
// --- Normal HUD ---
